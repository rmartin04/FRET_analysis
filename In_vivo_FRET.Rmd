...
gather_fluorescence <- function(n, x, y) {
  gather(n, Time, Fluorescence, x:y) %>%
    group_by(Time) %>%
    summarise(Fluorescence = mean(Fluorescence))
}

summarise_fluorescence <- function(n, x, y, z) {
  gather(n, Time, Fluorescence, x:y) %>%
...
setwd("~/Desktop/Practice_R/Jace_Projects/Ryan_Data2/2019-03-08")
...
control_cfp <- lapply(CFP[c(11:12, 23:24)] , gather_fluorescence, x = "Event001", "Event020") %>%
  reduce(left_join, by = "Time") %>%
  
  ## Adjust "* 2" for frequency of readings taken, here it is for every 2 minutes
...
control_yfp <- lapply(YFP[c(11:12, 23:24)], gather_fluorescence, x = "Event001", "Event020") %>%
  reduce(left_join, by = "Time") %>%
...
CFP_treat <- lapply(CFP[c(1:10, 13:22)], summarise_fluorescence, x = "Event001", y = "Event020", z = 1)
...
YFP_treat <- lapply(YFP[c(1:10, 13:22)], summarise_fluorescence, x = "Event001", y = "Event020", z = 1)
